...
HASH_CHUNK_SIZE = 65536  # 64 KiB – balances memory and I/O for hashing
...
def file_checksum(filepath: str) -> str:
    ...
    try:
        ...
        logging.warning("Could not hash %s: %s", filepath, exc)
        return ""
...
def files_are_equal(src: str, dst: str) -> bool:
    ...
    if not src_meta or not dst_meta:
        return False
    # Fast path – identical size *and* mtime means almost certainly the same.
    if (
    ...
    if src_meta["size"] != dst_meta["size"]:
        return False
    # Same size but different mtime → compare content hashes to be sure.
...
def load_manifest(backup_dir: str) -> dict:
    ...
    try:
        ...
        logging.warning(
            "Corrupt or unreadable manifest – starting fresh: %s", exc
        )
        return {"files": {}, "dirs": []}
...
def scan_source(source_dir: str):
    """Walk *source_dir* and return (set_of_rel_dirs, dict_of_rel_files).

    Directories are returned as a set of relative paths (including empty ones).
    Files are returned as a dict: {relative_path: absolute_path}.
    """
    rel_dirs = set()
    rel_files = {}
...
def perform_backup(source_dir: str, backup_dir: str) -> dict:
    ...
    current_dirs, current_files = scan_source(source_dir)
    new_file_manifest = {}
    ...
    for rel_dir in sorted(current_dirs):
        ...
        if not os.path.isdir(dst_dir):
            try:
                ...
                logging.error("Failed to create directory %s: %s", dst_dir, exc)
    ...
    for rel_path, src_path in current_files.items():
        ...
        try:
            if not os.path.exists(dst_path):
                ...
                # Existing file that has changed.
                shutil.copy2(src_path, dst_path)
            ...
            logging.error("Failed to copy %s → %s: %s", src_path, dst_path, exc)
            summary["errors"].append(rel_path)
    ...
    for rel_dir in sorted(deleted_dirs, key=lambda p: p.count(os.sep), reverse=True):
        dst_dir = os.path.join(backup_dir, rel_dir)
        try:
            ...
            logging.error("Failed to remove directory %s: %s", dst_dir, exc)
            summary["errors"].append(rel_dir + "/")

    # ------------------------------------------------------------------
    # Phase 5: Prune any leftover empty directories not in source
    # ------------------------------------------------------------------
...
def _prune_empty_dirs(backup_dir: str, source_dirs: set) -> None:
    ...
    Walks bottom-up so that nested empty dirs are cleaned correctly.
    """
    for dirpath, dirnames, filenames in os.walk(backup_dir, topdown=False):
        ...
        # Keep directories that exist in the source (even if empty)
        if rel_dir in source_dirs:
            continue
        # Only remove if truly empty (manifest file doesn't count)
        remaining = [n for n in (filenames + dirnames) if n != MANIFEST_FILENAME]
        if not remaining:
            try:
                os.rmdir(dirpath)
                logging.debug("Pruned empty dir: %s", dirpath)
            except OSError:
                pass
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    # Resolve to absolute paths for clarity in logs
    ...
    # Ensure backup dir is not inside source dir (would cause recursion)
...
def configure_logging(log_file: str = None, verbose: bool = False) -> None:
    """Set up the root logger with stderr + optional file handler."""
...
def main() -> int:
    ...
    while not _shutdown_requested:
        ...
        # Sleep in small increments to react to shutdown signals quickly.
        deadline = time.monotonic() + args.interval
        while time.monotonic() < deadline and not _shutdown_requested:
            time.sleep(min(1, deadline - time.monotonic()))

    logging.info(
        "Backup service stopped gracefully after %d cycle(s).", cycle
    )
    return 0
